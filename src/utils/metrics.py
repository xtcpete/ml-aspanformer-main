...
def relative_pose_error(T_0to1, R, t, ignore_gt_t_thr=0.01):
...
def symmetric_epipolar_distance(pts0, pts1, E, K0, K1):
    ...
    pts0 = convert_points_to_homogeneous(pts0).float()
    pts1 = convert_points_to_homogeneous(pts1).float()
    E = E.float()
...
def compute_symmetrical_epipolar_errors(data):
    ...
    for bs in range(Tx.size(0)):
        ...
        K1 = data['K1'][bs]
        epi_errs.append(
            symmetric_epipolar_distance(pts0[mask], pts1[mask], E_mat[bs], K0, K1))
...
def aggregate_metrics(metrics, epi_err_thr=5e-4):
    ...
    pose_errors = np.max(np.stack([metrics['R_errs']]), axis=0)
